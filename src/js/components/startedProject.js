...
function StartedProject() {
  ...
  const getProjects = async () => {
    try {
      ...
      const projectsWithStartedClassification = [];

      for (const project of userProjects) {
        const classifications = await getProjectClassifications(project.id);
        const startedClassification = classifications.find(
          (classification) => classification.title === "Started"
        );

        if (startedClassification) {
          projectsWithStartedClassification.push(project);
        }
      }

      setProjects(projectsWithStartedClassification);
    } catch (error) {
  ...
  const handleStartProject = async (project) => {
    ...
    if (startedClassification) {
      try {
        await api.put(`/projects/${project.id}/classifications/${startedClassification.id}`, {
          title: "Done",
      ...
      console.error(`Classificação 'Started' não encontrada para o projeto ${project.id}.`);
    }
  };
          ...
          {Array.isArray(projects) && projects.length > 0 ? (
            projects.map((project) => (
              <div className="task projectContainer" key={project.id}>
                ...
                <button className="btnFazer" onClick={() => handleStartProject(project)}>Concluir</button>
              </div>
            ))
          ) : (
